...
def expand_model_specs(model_patterns: List[str]) -> List[str]:
    ...
    if not models:
        raise click.ClickException(f"No model specification files found for patterns: {model_patterns}")
...
def initialize_results_file(results_dir: str) -> str:
    ...
    headers = [
        "model_spec", "run", "rt_s", "mem_mb", 
        "gather_results_cumtime", "gather_results_percall", "gather_results_ncalls",
        "pipeline_call_cumtime", "pipeline_call_percall", "pipeline_call_ncalls",
        "population_get_cumtime", "population_get_percall", "population_get_ncalls"
    ]
...
def parse_function_metrics(stats_file_txt: str, pattern: str) -> Tuple[Optional[float], Optional[float], Optional[int]]:
    ...
    try:
        ...
        logger.warning(f"Could not parse function metrics from {stats_file_txt} with pattern {pattern}: {e}")
...
def run_single_benchmark(
    ...
    gather_results_cumtime, gather_results_percall, gather_results_ncalls = parse_function_metrics(
        stats_file_txt, r"results/manager\.py:\d+\(gather_results\)"
    )
    
    pipeline_call_cumtime, pipeline_call_percall, pipeline_call_ncalls = parse_function_metrics(
        stats_file_txt, r"values/pipeline\.py:\d+\(__call__\)"
    )
    
    population_get_cumtime, population_get_percall, population_get_ncalls = parse_function_metrics(
        stats_file_txt, r"population/population_view\.py:\d+\(get\)"
    )
...
def write_results_to_csv(results_file: str, results: Dict[str, Any]) -> None:
    ...
    with open(results_file, 'a', newline='') as f:
        writer = csv.writer(f)
        row = [
            results['model_spec'], results['run'], results['rt_s'], results['mem_mb'],
            results['gather_results_cumtime'], results['gather_results_percall'], results['gather_results_ncalls'],
            results['pipeline_call_cumtime'], results['pipeline_call_percall'], results['pipeline_call_ncalls'],
            results['population_get_cumtime'], results['population_get_percall'], results['population_get_ncalls']
        ]
        writer.writerow(row)
